# BACnet B-DAP (Device Address Proxy) - C++ example

A minimal, copy-paste-friendly example showing how to implement the BACnet
**B-DAP (BACnet Device Address Proxy)** device profile in C++ using the
...
per BACnet device profile. This example claims **only** B-DAP.
...
> **v2.3.0**. Running the example prints all three - if what it prints
> disagrees with this line, trust the program and check `CHANGELOG.md`.

B-DAP's required BIBB set (DS-RP-B, DS-WP-B, DM-DDB-B, DM-DOB-B) is identical to
[B-SA (Smart Actuator)](https://github.com/chipkin/BACnetProfileExample-B-SA-CPP)'s,
so the object model and service wiring below are the same: three read-only
sensor inputs plus three commandable outputs a client drives with
WriteProperty. B-DAP additionally requires **DM-DAB-B** (Dynamic Address
Binding - proxying address-binding requests on behalf of other devices); see
**TODO.md** for why this example does not implement it.

## What is a B-DAP (BACnet Device Address Proxy) profile?
...
**B-DAP (BACnet Device Address Proxy)** is a Miscellaneous-family profile (Annex
L.7): a device that, in addition to the DS/DM baseline, proxies address-binding
requests on behalf of other devices on a directly-connected network.
...
- **Data Sharing - WriteProperty - B side (DS-WP-B):** the device must accept
  **WriteProperty** requests so a controller can drive its outputs.
- **Discovery:** the device must be findable, so it answers **Who-Is** with
  **I-Am** and **Who-Has** with **I-Have** (DM-DDB-B, DM-DOB-B), and announces
  itself with an unsolicited I-Am at start-up.
- **Device Management - Dynamic Address Binding - B side (DM-DAB-B):** the
  device proxies address-binding requests on behalf of other devices. **Not
  implemented in this example** - see `TODO.md`.
...
## Commandable outputs (demonstrating DS-WP-B)
...
```
Device 389021  "Rainbow"   (Vendor 389 - Chipkin Automation Systems)
    │
...
Fuchsia, Indigo) demonstrate DS-WP-B; their `Present_Value` starts at the
...
is the point of a profile example. These capabilities satisfy four of the five
BIBBs the **B-DAP (BACnet Device Address Proxy)** profile requires; DM-DAB-B is
not implemented (see `TODO.md`). Because DS-RP-B/DM-DDB-B/DM-DOB-B are a
...
| DM-DAB-B | Device Management - Dynamic Address Binding - B | ⏳ not implemented — see `TODO.md` |
...
| `MODEL_NAME` | `CAS BACnet Stack Example - B-DAP` | Your model designation. This is what a building operator reads to identify your device. |
...
| Device instance | `389021` (`--deviceID` overrides) | Must be unique on the internetwork. BACnet requires this to be configurable; keep it so. |
...
```bash
git clone --recursive https://github.com/chipkin/BACnetProfileExample-B-DAP-CPP.git
cd BACnetProfileExample-B-DAP-CPP
...
```bash
tools/build-stack-static.sh BACnetProfileExample-B-DAP-CPP   # from the series root; builds
...
```bash
# Linux / macOS
./build/BACnetExampleBDAP

# Windows
.\build\Release\BACnetExampleBDAP.exe
```
...
| `--deviceID <n>` | `389021` | The device's BACnet instance number (BACnet requires this to be configurable). |
...
1. **Discover** - send a **Who-Is**. The device replies with **I-Am** from
   instance **389021** (vendor **389**). It also broadcasts an I-Am at start-up.
...
3. **Read the Device** - ReadProperty `389021` -> `Object_Name` returns
...
5. **Command an output (the DS-WP-B test)** - ReadProperty Analog Output `1`
...
7. **Discover by object (DM-DOB-B)** - send a **Who-Has** naming Analog Input 1
   (`"Bronze"`). The device replies with **I-Have**. `Protocol_Services_Supported`
   read from the Device confirms `who-has` and `i-have` are both enabled.
8. **DM-DAB-B is not exercised here** - this example does not proxy
   address-binding requests for other devices; see `TODO.md` for why.
...
|---|---|---|---|---|---|---|---|
| Windows x64 (windows-2022) | `BACnetExampleBDAP.exe` | 3,250,688 bytes (~3.1 MiB) | `bea309274ab89868` | 191 ms | `abd4cee1` | STATIC | Visual Studio 17 2022 |
| Linux x64 (ubuntu-latest) | `BACnetExampleBDAP` | 44,528 bytes (~43 KiB) | `2f54ac276f689c16` | 109 ms | `abd4cee1` | STATIC | `/usr/bin/c++` |

From release [v1.0.0](https://github.com/chipkin/BACnetProfileExample-B-DAP-CPP/releases/tag/v1.0.0) (`metrics-windows.json` / `metrics-linux.json`).
