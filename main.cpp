...
// =============================================================================
// BACnet Profile Example - B-DAP (BACnet Device Address Proxy) - C++
//
// This example implements the BACnet "B-DAP" (Device Address Proxy) device
// profile with the CAS BACnet Stack. Its required BIBB set - DS-RP-B, DS-WP-B,
// DM-DDB-B, DM-DOB-B - is identical to B-SA's, so the object model and service
// wiring below are unchanged from that example; only identity differs.
//
// A B-DAP (ANSI/ASHRAE 135, Annex L.7 Miscellaneous) is required to support:
//
//     DS-RP-B  - respond to ReadProperty requests, and
//     DS-WP-B  - allow other devices to WriteProperty its objects
//
// plus discovery (Who-Is/I-Am, Who-Has/I-Have) and DM-DAB-B (Dynamic Address
// Binding - proxying address-binding requests on behalf of other devices on a
// directly-connected network). DM-DAB-B is NOT implemented here - see
// TODO.md: the pinned CAS BACnet Stack (6.0.21, commit abd4cee1) has no
// customer-facing export for it (stack issue #2032). It does NOT require COV,
// alarms, scheduling, or trending, so this example leaves those off.
//
// The device keeps the three read-only sensor objects and three commandable
// OUTPUT objects that a client drives with WriteProperty. Each object
// has a colour name (the convention shared across this example series):
//
//     Device 389021            "Rainbow"     (instance configurable with --deviceID)
...
// -----------------------------------------------------------------------------
static const char* APP_NAME = "BACnet B-DAP (Device Address Proxy) Example - C++";
static const char* APP_VERSION = "1.0.0";

// The device instance. BACnet requires this to be configurable, so it defaults
// to 389021 (each profile example in the series has its own default, so several
// examples can run on one subnet) and can be overridden with --deviceID. Keep it
// configurable in your product: it must be unique across the internetwork.
static uint32_t g_deviceInstance = 389021;
...
static const char* DEVICE_DESCRIPTION =
    "Chipkin CAS BACnet Stack example - B-DAP (Device Address Proxy) profile. "
    "Demonstrates DS-RP-B + DS-WP-B: ReadProperty and WriteProperty with "
    "read-only sensor inputs and commandable outputs, plus DM-DDB-B/DM-DOB-B "
    "discovery. DM-DAB-B is not implemented; see TODO.md.";
...
static const char* MODEL_NAME = "CAS BACnet Stack Example - B-DAP";
...
// demonstrate DS-WP-B: clients drive them with WriteProperty.
...
// -----------------------------------------------------------------------------
// 2b. Property "set" callbacks - implement DS-WP-B
//
...
int main(int argc, char** argv) {
    ...
    // The "set" callbacks implement DS-WP-B: they accept WriteProperty
    ...
    // Enable the two services B-DAP must execute: ReadProperty (DS-RP-B) and
    ...
    // alarm/event service: B-DAP does not require them, so a faithful example
    // leaves them off. DM-DAB-B (the proxy BIBB) is also not enabled here -
    // see TODO.md.
    ...
    // --- Add the commandable OUTPUT objects (demonstrate DS-WP-B) ------------
